...
var event2NodeTemplate =
    $(go.Node, "Vertical",
      ...
      new go.Binding("location", "loc", go.Point.parse).makeTwoWay(go.Point.stringify),
      // move a selected part into the Foreground layer, so it isn't obscured by any non-selected parts
       new go.Binding("layerName", "back"),
      new go.Binding("layerName", "isSelected", function(s) { return s ? "Background" : ""; }).ofObject(),
      // can be resided according to the user's desires
      { resizable: true, resizeObjectName: "SHAPE" },
        $(go.Panel, "Spot",
...
$(go.Shape, "NotAllowed",
 ...
 var poolGroupTemplate =
  ...
  nodeTemplateMap.add("event", eventNodeTemplate);
      nodeTemplateMap.add("event2", event2NodeTemplate);
  ...
  palNodeTemplateMap.add("event", eventNodeTemplate);
      palNodeTemplateMap.add("event2", event2NodeTemplate);
